...
// Create user
const createNewUser  = () => {
    // https://koders1gpython-default-rtdb.firebaseio.com/brian
...
const readUser  = () => {
    ...
    // xhttp.open('GET', 'https://koders1gpython-default-rtdb.firebaseio.com/brian/users/-MQnTbsClYPUsYDcBS-R.json')
    xhttp.send()
}
    ...
    document.querySelectorAll(".btn__delete__user").forEach((btn)=> {
        ...
        btn.addEventListener('click', function(user) {
            ...
            if(user.target.classList.contains('btn__delete__user')) {
                ...
                // request
                requestAjax(
                    `https://koders1gpython-default-rtdb.firebaseio.com/brian/users/${idUser}.json`,
                    redirectToUsers,
                    'DELETE'
                )
            }
    ...
    for(user in response ) {
        ...
        usersList += `
            <li class="d-flex px-3 py-2 shadow-sm mb-2 rounded justify-content-between item__user"> 
                <div class="d-flex"> 
                    <span class="d-flex rounded-circle icon__user" style='width: 50px; height: 50px; background-image:  url(${dataUsuario.urlPhoto})'></span> 
                ...
                <div class="d-flex"> 
                    ...
                    <a href="update-user.html?id=${user}" class="btn-success btn__update__user" data-id="${user}" data-user="${user}">Update</a>    
                </div> 
            </li>
        `
    }
